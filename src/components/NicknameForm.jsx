...
const NicknameForm = ({ score, totalQuestions, category, onSubmit }) => {
  ...
  return (
    <motion.div 
      ...
      className={`w-full max-w-md mx-auto bg-gradient-to-b ${theme.bgFrom} ${theme.bgTo} p-8 rounded-2xl backdrop-blur-sm ${theme.border} shadow-xl`}
    >
      <h2 className="text-2xl font-bold text-center mb-1 text-white">Mission Complete!</h2>
      
      <div className="my-6 flex flex-col items-center">
        <div className="relative mb-3">
          <div className={`w-32 h-32 rounded-full bg-gradient-to-b from-${theme.primary}-600/20 to-${theme.primary}-600/20 flex items-center justify-center border-4 ${theme.border}`}>
            <div className="text-4xl font-bold text-white">{percentage}%</div>
          </div>
          <div className={`absolute -top-2 -right-2 bg-${theme.primary}-900/80 ${theme.border} rounded-lg px-2 py-1 text-xs font-bold text-white flex items-center`}>
        ...
        <div className="holographic-text mb-1">
          <span className={`text-xl font-bold text-${theme.primary}-400`}>{rank}</span>
        </div>
      ...
      <form onSubmit={handleSubmit} className="mt-6">
        <div className="mb-5 relative">
          <label className={`block text-sm text-${theme.primary}-300 mb-2 font-medium`} htmlFor="nickname">
          ...
          <div className="relative">
            <input
              ...
              maxLength={20}
              className={`w-full bg-${theme.primary}-900/30 border ${theme.border} focus:border-${theme.primary}-400 rounded-lg px-4 py-3 text-white placeholder-${theme.primary}-300/50 focus:outline-none focus:ring-2 focus:ring-${theme.primary}-500/40`}
            />
            <div className="absolute inset-0 rounded-lg pointer-events-none holographic-scanner"></div>
          </div>
          <div className="text-right mt-1">
            <span className={`text-xs text-${theme.primary}-400/70`}>{nickname.length}/20</span>
          </div>
        </div>
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          type="submit"
          className={`w-full py-3 bg-gradient-to-r ${theme.gradient} rounded-lg text-white font-bold transition-all relative overflow-hidden group`}
        >
